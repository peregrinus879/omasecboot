...
# Limine 12 checks every path hash that is present, whatever the protocol,
# and panics on a mismatch once Secure Boot and config enrollment are both
# active; only an absent hash is waived for EFI chainloading (uri.c and
...
list_limine_entry_paths() {
  ...
  awk -v mode="$mode" '
    ...
    function flush_entry(    i) {
      if (mode == "unhashed" && (protocol == "efi" || protocol == "efi_boot_entry")) {
        reset_entry()
        return
      }
...
# Hashed paths whose file no longer matches, or that cannot be resolved
...
# Runs one pinned Limine tool through its validated open inode. The evidence
# document names the tool's path, identity, and hash; both are checked again
# on the open descriptor immediately before execution.
run_bound_limine_tool() {
  local key="$1" handoff="$2" tools_json="$3" path expected_identity expected_hash
  local tool_fd fd_path rc=0
  shift 3
  [[ "$key" == install || "$key" == mkinitcpio || "$key" == reset \
    || "$key" == snapper-sync ]] || return 1
  [[ "$handoff" == true || "$handoff" == false ]] || return 1
  path=$(jq -r --arg key "$key" '.[$key].path' <<< "$tools_json") || return 1
  expected_identity=$(jq -r --arg key "$key" '.[$key].identity' \
    <<< "$tools_json") || return 1
  expected_hash=$(jq -r --arg key "$key" '.[$key].sha256' <<< "$tools_json") || return 1
  [[ "$path" == /* ]] || return 1
...
run_bound_unconfigure_limine_tool() {
  local key="$1" handoff="$2"
  shift 2
  [[ "$key" == install || "$key" == mkinitcpio || "$key" == reset ]] || return 1
  unconfigure_limine_tools_match_intent || return 1
  run_bound_limine_tool "$key" "$handoff" "$_unconfigure_limine_tools_json" "$@"
}

# Activation regenerates Limine entries through the pinned limine-mkinitcpio
# and limine-snapper-sync (both hooks admit a child of the owning transaction).
activation_limine_tools_are_pinned() {
  local path
  unconfigure_limine_tools_are_pinned || return 1
  [[ $(producer_package_version limine-snapper-sync) == \
    "$SUPPORTED_LIMINE_SNAPPER_SYNC_VERSION" ]] || return 1
  path=$(limine_snapper_sync_path)
  validate_control_file "$path" || return 1
  [[ -x "$path" ]] || return 1
  [[ $(producer_file_owner_package "$path") == limine-snapper-sync ]]
}

capture_activation_limine_tools() {
  local document mkinitcpio snapper
  activation_limine_tools_are_pinned || return 1
  mkinitcpio=$(pinned_tool_evidence_json "$(limine_mkinitcpio_path)") || return 1
  snapper=$(pinned_tool_evidence_json "$(limine_snapper_sync_path)") || return 1
  document=$(jq -cn --argjson mkinitcpio "$mkinitcpio" --argjson snapper "$snapper" \
    '{mkinitcpio: $mkinitcpio, "snapper-sync": $snapper}') || return 1
  printf '%s\n' "$document"
}

# Entries written under ENABLE_VERIFICATION=yes carry hashes of the unsigned
# files; signing in place would leave them stale, and Limine 12 panics on a
...
regenerate_limine_snapshot_entries() {
  ...
  run_visible run_bound_limine_tool snapper-sync true "$_activation_limine_tools_json" \
    --no-force-save || {
    fail "limine-snapper-sync failed while regenerating the snapshot entries"
    return 1
  }
  durable_sync "$(esp_path)"
}
...
# The repair path ("repair", used by sign and the Windows operations) enrolls
# the current config checksum, cleans tracking, signs, and proves. Activation
# ("activation") also regenerates the Limine entries: the OS entry before
# signing and the snapshot entries after it, so the checksum enrolled last
# describes the final config and no hash describes an unsigned file.
repair_boot_artifacts() {
  local mode="${1:-repair}" binary file
  local -a binaries=()
  [[ "$mode" == repair || "$mode" == activation ]] || return 1
  ...
  if [[ "$mode" == activation ]]; then
    transaction_phase_start "regenerate-entries" || return 1
    regenerate_limine_os_entry || return 1
    transaction_phase_complete "regenerate-entries" || return 1
    ...
    transaction_phase_start "regenerate-snapshot-entries" || return 1
    regenerate_limine_snapshot_entries || return 1
    artifact_repair_refresh_config_checksum || return 1
    transaction_phase_complete "regenerate-snapshot-entries" || return 1
  fi
