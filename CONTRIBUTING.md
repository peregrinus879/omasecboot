...
- `limine-hooks/90-omasecboot-sign`: the only hook. `systemd/omasecboot-watch@.path` and `.service`: the watcher templates; `setup` enables one instance for `limine.conf` and one for the primary loader. `omarchy/omarchy-menu.jsonc`: the reference menu row for "Reboot to Windows". `PKGBUILD`, `Makefile`.
...
- `.github/workflows/ci.yml`: lint, the hermetic suites, the package build and `tests/container.sh`; `contracts.yml`: the scheduled contract run. `.github/ISSUE_TEMPLATE/field-report.yml`: the report form of `docs/field-testing.md`; the two change together.
