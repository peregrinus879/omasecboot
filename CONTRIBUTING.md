...
- `tests/lib/harness.sh`: the fixture machine and the stub tools; `tests/lib/esl.sh`: signature-list builders shared with the sbctl stub. `tests/*.sh`: the suites `common`, `limine`, `sign`, `status`, `firmware`, `windows`, `commands`, `install` and `package`; `tests/container.sh` runs as root in a disposable container only; `tests/acceptance-record.sh` records hardware acceptance rows.
