...
- `tests/lib/harness.sh`: the fixture machine and the stub tools; `tests/lib/esl.sh`: signature-list builders shared with the sbctl stub; `tests/lib/transcript.sh`: what the recorder keeps out of a record. `tests/*.sh`: the hermetic suites `common`, `limine`, `sign`, `status`, `firmware`, `windows`, `commands`, `records`, `install`, `package` and `documents`; `tests/contract-sbctl.sh` and `tests/contract-limine.sh` run the installed tools in the sandbox of `tests/lib/sandbox.sh`; `tests/container.sh` runs as root in a disposable container only; `tests/acceptance-record.sh` records hardware acceptance rows, and `tests/acceptance-share.sh` makes the copies of them that are fit for a public issue.
