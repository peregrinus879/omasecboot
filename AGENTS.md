...
The current implementation provides the lifecycle boundary, tested artifact-proof transaction, bounded producer recovery with expected-EFI obligations, bounded firmware-trust recovery, a dormant BootNext transaction with immutable pre-write evidence and direct readback, validated Windows target identity, read-only Windows encryption preflight, raw firmware backup, strict trust planning, five-state classification, and dormant enrollment failure proof. It deliberately reports consolidated repair, firmware-enrollment, and Windows-mutation capability unavailable until the remaining recovery domains land. `setup`, `enroll`, `sign`, `cleanup`, public Windows mutation, active producer automation, and uninstall remain blocked. The remaining contracts are mandatory for the package-first release and must not be described as shipped until their implementation and tests land.
