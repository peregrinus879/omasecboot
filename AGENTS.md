...
- `producers.sh` - ancestor-bound producer leases, fixed reconstruction/recovery registry, and expected-EFI obligations
...
The current implementation provides the lifecycle boundary, tested artifact-proof transaction, bounded producer, firmware, Windows, software, and unconfiguration recovery, dormant BootNext mutation with immutable evidence and direct readback, validated Windows target identity, read-only Windows encryption preflight, raw firmware backup, strict trust planning, five-state classification, dormant enrollment failure proof, and gated public-command integration. It deliberately reports consolidated repair, firmware enrollment, Windows mutation, active producer automation, and uninstall unavailable until the T-6.8 activation and later packaging units land. `setup`, `adopt`, `enroll`, `sign`, `cleanup`, `unconfigure`, `repair`, public Windows mutation, active producer automation, and uninstall remain blocked. The remaining contracts are mandatory for the package-first release and must not be described as shipped until their implementation and tests land.
...
- A Limine pre-hook rejects external hook-aware mutation during an OmaSecBoot transition. A post-hook validates and locks inherited FD 200 or acquires the shared lock itself before repair. ALPM PreTransaction guards block boot-mutating package transactions during `transition` and `recovery-required`, and preserve recovery dependencies until state is verified `disabled` or pristine. The removal classifier runs under both boot locks; generic lifecycle mutation checks the canonical pacman database lock after lock acquisition and again immediately before transition publication.
...
- Producer completion requires final-proof schema 2 with immutable expected-EFI obligations. Historical schema-1 final proofs remain readable for existing non-producer transactions. The pinned `limine-mkinitcpio-hook` inventory comes from package ownership of `/usr/lib/modules/*/modules.builtin`; the pinned snapshot inventory comes from every EFI `fileHashName` in manifest schema 1.3.0. Derive obligations again after proof and fail closed on inventory races or missing outputs.
...
- Keep consolidated repair capability unavailable until interrupted recovery can handle every mutation domain. Artifact proof alone does not authorize producer automation, and a PostTransaction backup cannot represent a package artifact's pre-transaction state.
- Enforce the captured active or inactive state of `limine-snapper-sync.service` only after durable transition publication, then restore that state before stable commit and on handled failure paths. Indeterminate pre-transaction state aborts cleanly. This remains auxiliary quiescing, not the concurrency boundary. Snapper plugins, transient units, cleanup, and full restore are separate producers. Full `limine-snapper-restore` has no parent shared lock on the audited release; allow it only in stable state, bind its runtime marker inode, serialize post-repair, and reject it during `transition` and `recovery-required`. Stale recovery removes the marker pathname only while it matches the recorded inode after exact restore-process inspection under both locks.
...
- The dormant BootNext transaction reads the raw six-byte efivarfs value, publishes immutable current-boot, prior-value, exact-target, and pinned-`efibootmgr` evidence before mutation, repeats every proof immediately before its only `-n` call, and requires exact direct readback plus a second complete target proof. Execute the validated open inode rather than reopening its pathname. Any uncertain post-publication outcome remains `recovery-required`; keep the production predicate and dispatcher closed until T-6.8 activates the integrated recovery path.
- Windows recovery admits only a validated `windows-bootnext` root and records its root write frontier, fresh boot ID, direct observation, planned action, and planned outcome before any repair. Resolve a root with no BootNext record as `not-published`; a record whose write phase was not reached only when the prior value remains exact; an observed prior as `prior-unchanged`; an observed target by restoring the exact prior; and later-boot absence after a possible write as `consumed-unknown`, never as proof Windows booted. Restore present values through the root-pinned open `efibootmgr` inode. Efibootmgr 18 `-N` is unusable because it mistakenly requires `Boot0001`, so restore absence through the pinned open GNU `unlink` inode after re-proving the exact efivarfs pathname, device, inode, and hash, then require direct absence readback. Unreadable or unrelated state remains `recovery-required`; keep the integrated public Windows mutation and recovery routes closed until T-6.8 activation.
...
- V1 has no dbx writer and supports only a confirmed PK-only firmware operation with present zero AuditMode and DeployedMode values. Missing mode variables, clear-all-only firmware, or changed KEK/db/dbx state blocks before enrollment. Root db, KEK, and PK writes remain dormant behind the unavailable firmware-enrollment predicate; check it at entry, before artifact mutation, and before every firmware write. The internal firmware-recovery predicate authorizes only validated continuation attempts and does not authorize root enrollment or a public dispatcher route.
