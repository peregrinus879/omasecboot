...
The current implementation provides the lifecycle boundary, tested artifact-proof transaction, bounded producer recovery with expected-EFI obligations, bounded firmware-trust recovery, a dormant BootNext transaction with immutable pre-write evidence and direct readback, bounded Windows BootNext recovery, validated Windows target identity, read-only Windows encryption preflight, raw firmware backup, strict trust planning, five-state classification, and dormant enrollment failure proof. It deliberately reports consolidated repair, firmware-enrollment, and Windows-mutation capability unavailable until the remaining lifecycle and command-integration units land. `setup`, `enroll`, `sign`, `cleanup`, public Windows mutation, active producer automation, and uninstall remain blocked. The remaining contracts are mandatory for the package-first release and must not be described as shipped until their implementation and tests land.
...
- Incident-chain recovery operations are exhaustive: producer roots admit only `producer-recovery`, `enroll-secure-boot` roots without producer evidence admit only `firmware-recovery`, and `windows-bootnext` roots admit only `windows-recovery`. Firmware recovery derives its authority from the root transaction's hashed prior lifecycle and that lifecycle's exact completed `activate-secure-boot-plan` transaction. It inherits the immediate predecessor's backup, plan, rollback policy, and append-only firmware ledger; resolves only a terminal pending write through direct F0-F3 readback; repairs and proves EFI artifacts before continuation; and completes only after F3, a second EFI verification, and transaction-local firmware and schema-2 artifact proofs. Technical readback uncertainty leaves pending evidence unresolved.
