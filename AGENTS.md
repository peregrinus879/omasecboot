...
- The BootNext transaction reads the raw six-byte efivarfs value, publishes immutable current-boot, prior-value, exact-target, and pinned-`efibootmgr` evidence before mutation, repeats every proof immediately before its only `-n` call, and requires exact direct readback plus a second complete target proof. Execute the validated open inode rather than reopening its pathname. Any uncertain post-publication outcome remains `recovery-required` for integrated Windows recovery.
