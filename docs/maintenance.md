...
- A lifecycle lock across `setup`, `remove` and the Windows commands, held through the intervals in which the boot lock is released around upstream's tools: the boot lock serialises the boot files and the pass looks again under it (spec, D2), while two of those commands run at once only by operator error.
...
- The recorder's own failures beyond the two it stops at (a record header or a transcript that cannot be written): a state block that fails half way, and a row whose "State after" was never written, should end the row with a status of the recorder's own.
