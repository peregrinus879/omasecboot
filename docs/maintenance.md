...
- The record of a stock single-boot Omarchy install, where the fallback loader and its `/EFI fallback` menu entry exist from the start. The recorded machine (C6 of [upstream-contracts.md](upstream-contracts.md)) was installed beside Windows and had neither.
...
| `limine-mkinitcpio-hook` | C2 and C3: hook names 89, 90 and 91 (ours sorts between the last two), the condition of hook 89, the loader backup `limine_x64.bak`, `limine-install --no-efi-register` and `--fallback`, when the fallback is deployed and that the step copies over whatever is there, the lock path and descriptor, the configuration layers |
| `limine-snapper-sync` | C2 and C8: history file names, `snapshots.json`, the restore marker, the files the restore command exports, what its rewrite of `limine.conf` keeps |
