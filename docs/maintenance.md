...
- limine-entry-tool and limine-snapper-sync use FD 200 and `/run/lock/boot-partition.lock`, but several audited wrappers continue after `mutex_lock` failure. Full `limine-snapper-restore` invokes `limine-snapper-sync --restore --no-mutex` without a parent shared lock. Stopping `limine-snapper-sync.service` does not quiesce Snapper plugins, transient units, or the cleanup service's `ExecStopPost`. The approved release treats service stopping as auxiliary only, validates inherited descriptor ownership plus pathname device and inode, calls `flock` on validated FD 200, rejects hook-aware external mutation during `transition` or `recovery-required`, serializes post-repair, allows full restore only in stable state, and uses ALPM PreTransaction guards for package producers.
- The current Limine post-hook trusts `OMASECBOOT_IN_LIMINE_HOOK=true` without proving ancestry, descriptor ownership, or lifecycle state. The target release uses the complete root-owned transaction-manifest schema in `docs/implementation-contract.md`; do not maintain a partial duplicate here. Stable state is committed last, rollback failure leaves `recovery-required`, and existing unrecorded configurations require explicit adoption.
