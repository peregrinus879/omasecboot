...
With Secure Boot on, the firmware starts an EFI program only when a certificate in its `db` signed it. The Platform Key (PK) says who may change the Key Exchange Keys (KEK), and KEK says who may change `db` and its revocation list `dbx`. A machine leaves the factory with the manufacturer's PK, and with Microsoft's certificates and usually the manufacturer's own in KEK and `db`, so Windows and anything those certificates signed start, and nothing else does.
...
The seal is why `limine.conf` and the loader must change together: an edit of `limine.conf` without a re-seal stops the machine at Limine's panic, with Secure Boot on or off, and a raw loader copied over the sealed one checks nothing, so the firmware refuses it once Secure Boot is on. It is also why a raw loader, one without a seal, is the rescue while Secure Boot is off: it starts whatever `limine.conf` holds.

The images themselves need no hash in `limine.conf`, because the firmware verifies each of them when Limine asks it to load one. A hash there would add nothing to that check and would go stale whenever the image is signed again in place.
...
"Are my certificates in the firmware?" and "is Secure Boot enforcing?" are separate questions with separate answers. Keys can be enrolled while Secure Boot is off, which is how `setup` proceeds, and Secure Boot can be on with the factory keys, which is how a machine arrives. `status` reports both, and `setup` asks for Secure Boot to be turned on only when the firmware starts the Limine loader through an entry of its own, since the fallback path is refused under Secure Boot.
...
Where the facts live: [section 4 of the spec](spec.md#4-architecture), [D9](spec.md#d9-keys-are-enrolled-by-appending), [D10](spec.md#d10-one-firmware-step-per-run), [C4](upstream-contracts.md#c4-sbctl-018), [C7](upstream-contracts.md#c7-firmware-boot-entries-efi_boot_entry-and-bootnext).
...
Two writers stand outside the chain: an editor on `limine.conf`, and the pacman hook that Omarchy's installer leaves, which copies a raw loader over the sealed one after every Limine upgrade. Two watchers, one on each file, run the pass when either changes. The pass rebuilds the loader only when its proof fails, and repeats until `limine.conf` held still.
...
"Raw" is what sbctl can tell: no seal and no signature by the current key. The spec states that limit, and the one that upstream's install redeploys the fallback by the user's own setting.
...
BitLocker checks no signature itself; it asks the TPM whether the boot measured as it did when the disk's key was sealed. The TPM records, in registers called PCRs, hashes of what the firmware ran and of the Secure Boot state and keys, and BitLocker releases the disk's key only when the registers read as they did when it was sealed. A change of PK, KEK, `db` or of Secure Boot's state changes those registers, which is why the recovery key must be at hand before any of them, and why the tool asks for it.
