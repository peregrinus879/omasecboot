...
JUDGMENT: Every top-level mutation writes a root-owned transaction manifest before changing persistent state. The manifest records its ID, boot ID, owner PID and process start time, prior stable state, operation, completed phases, backup paths and hashes. Stable lifecycle state is committed last.

FACT: The package-first implementation has not shipped a lifecycle schema to users, so schema 2 starts without an in-place migration. Every reader, including the removal classifier, rejects every other schema; no legacy document authorizes removal or mutation.
...
JUDGMENT: T-6.1 validates attempt manifests and seals as a chain linked to the immutable root and immediately preceding attempt. The reader accepts at most 32 attempts, and the read-only capacity check rejects another attempt before any recovery writer is called. Incident reads classify evidence as `absent`, `supported`, `unsupported-schema`, `attempt-limit`, `malformed`, or `control-state-ambiguous`. T-6.7 fixes consolidated dispatch to five operation-selected domains: producer, firmware, Windows BootNext, supported software rollback, and preserved unconfiguration. Each public mutation command performs only root and recovery-infrastructure checks before registry recovery, returns after any recovery, and evaluates its own interactive, firmware, ESP, or mutation dependencies only before fresh work. T-6.8 activates this completed recovery boundary.
...
JUDGMENT: Existing installations require an explicit adoption flow. It displays the existing managed settings and records every user-confirmed original value; public adoption rejects `unknown` rather than inventing the pre-OmaSecBoot state or publishing an `active` lifecycle that automatic unconfiguration cannot reverse. Historical lifecycle records may still contain `unknown`, and automatic `unconfigure` continues to reject them. Setup and adoption may publish `active` only after proving the exact supported recovery packages, pinned unconfiguration tools, and all five installed hooks against their canonical content and command target.
...
JUDGMENT: OmaSecBoot does not stop or start `limine-snapper-sync.service`. The shared lock is the concurrency boundary; Snapper plugins and cleanup can launch independent syncs, and each is admitted as its own producer.

JUDGMENT: Permit full snapshot restore only in stable state while the upstream runtime marker exists, serialize post-repair, and reject it during OmaSecBoot transitions. No other lease, stale-transition reconciliation, or producer recovery runs while the marker exists, and OmaSecBoot never removes it; `/run` is ephemeral, so a reboot clears a marker left by a crashed restore. Upstream's mutation window itself is not serialized until upstream removes the `--no-mutex` path.

JUDGMENT: Independent hooks under an external package or Limine coordinator authorize producer suppression and completion through the immutable producer record, exact coordinator identity, process start time, boot ID, and current ancestry. A producer lease binds that coordinator, invocation class, and fixed registry subtype before mutation. Every recognized Limine producer acquires or recovers the coordinator's FD 200 plus the repair lock and re-reads lifecycle state before either inactive no-op or active lease publication; inactive non-restore producers reject an existing full-restore marker. Nested reconstruction may inherit FD 201 only when the current pathname and parent descriptor identity agree; child release closes its inherited descriptor without unlocking the parent's open file description. Exit and signal handlers reacquire delegated locks before durable rollback or incident publication.
...
JUDGMENT: Artifact proofs use final-proof schema 2: the fresh discovered, signed, and tracked artifact set, which a completed producer transaction requires. An in-progress producer manifest accepts only an exact registered phase prefix with `current_phase` equal to its next phase; completion requires the complete reconstruction, artifact-repair, and proof sequence.

JUDGMENT: The `limine-mkinitcpio-hook` and `limine-snapper-sync` producers are version-pinned. Recovery of an interrupted package or UKI-build producer reruns `limine-mkinitcpio`, and recovery of a snapshot or restore producer reruns `limine-snapper-sync --no-force-save`, before artifact repair; normal completion trusts the producer's own output and proves every discovered artifact.
...
JUDGMENT: T-6.4 implements one root `windows-bootnext` transaction, activated for the public dispatcher by T-6.8. Before mutation it reads the raw efivarfs value as four-byte attributes plus one little-endian UINT16, records exact absence or value with the current boot ID, validated Windows target, and pinned `efibootmgr` package and executable hash, and publishes that create-once record through the transaction manifest. Immediately before the only `efibootmgr -n` call it proves the boot ID, prior BootNext value, executable hash, and all four target fields still match. Package ownership and the open executable inode are validated; that inherited descriptor supplies both the final target query and write so pathname replacement cannot change the invoked program. Command success is insufficient: exact raw readback must pass before stable commit. A command failure, uncertain readback, evidence race, or interruption after the write remains `recovery-required`; T-6.5 owns resolution, including the distinct consumed-unknown outcome.
...
JUDGMENT: Restoring a present prior value uses the root-bound `efibootmgr` executable, package-owned, hash-bound, and invoked through its validated open descriptor; efibootmgr is a version floor (18 or newer). Because efibootmgr 18's `-N` path mistakenly treats its boolean delete flag as a boot number and therefore depends on `Boot0001`, restoring recorded absence removes the one validated BootNext efivarfs file with GNU `rm` after re-proving the variable's exact pathname, device, inode, and hash, then requires direct absence readback. Command status is recorded, but exact readback is authoritative. Interrupted attempts are sealed and retried from fresh observation; persistent failures remain bounded by the incident attempt limit and require a future explicit recovery procedure after exhaustion. A completed stale attempt is published without replay.
...
JUDGMENT: A Microsoft boot manager on an internal ESP is a Windows signal only. OmaSecBoot does not inspect signer metadata, and no loader observation proves firmware bootability, db acceptance, or dbx status. Direct Windows boot, recovery-key preparation, and post-boot BitLocker verification remain mandatory.
...
4. **T-4 `feat: add the Windows encryption preflight`**: edition and management gate, Home decryption, Pro and higher suspension, ESP loader detection, and no-NTFS tests.
...
7. **T-6.2 `feat: recover boot artifact producers`**: registry-selected producer recovery, ancestor-bound producer leases, failed-package handling, inherited repair-lock safety, immutable artifact proof, and serialized repair coverage for package, Limine, snapshot, and restore producers.
...
16. **T-9 `ci: verify shell and package builds`**: lint, per-suite hermetic jobs, package build, and the privileged container package check.
