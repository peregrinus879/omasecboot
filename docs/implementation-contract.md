...
JUDGMENT: Existing installations require an explicit adoption flow. It displays the existing managed settings and records every user-confirmed original value; public adoption rejects `unknown` rather than inventing the pre-OmaSecBoot state or publishing an `active` lifecycle that automatic unconfiguration cannot reverse. Historical lifecycle records may still contain `unknown`, and automatic `unconfigure` continues to reject them. Setup and adoption may publish `active` only after proving the exact supported recovery packages, pinned unconfiguration tools, and all six installed hooks against their canonical content and command target.
