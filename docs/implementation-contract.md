...
FACT: The package-first implementation has not shipped a lifecycle schema to users, so schema 2 starts without an in-place migration. Normal readers reject every other schema. The removal classifier has one read-only exception for an exact historical schema-1 `disabled` record whose completed transaction manifest and recorded backup hashes validate; no other legacy document authorizes removal or mutation.
