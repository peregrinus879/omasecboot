...
JUDGMENT: Artifact proofs use final-proof schema 2. It binds one immutable obligation object to the fresh discovered, signed, and tracked artifact set, and a completed producer transaction requires it. An in-progress producer manifest accepts only an exact registered phase prefix with `current_phase` equal to its next phase; completion requires the complete sequence. Package producers prepend `package-pre-sbctl` to the common reconstruction, artifact-repair, proof, and output-confirmation sequence. `not-applicable` has no paths; `uki-inventory` contains every UKI required by the pinned producer; `snapshot-manifest` contains every hashed EFI filename referenced recursively by the pinned manifest schema. Paths are bounded, sorted, unique, and case-insensitively unique.
