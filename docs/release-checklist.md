...
The [release-critical roadmap](maintenance.md#release-critical-work) owns implementation outcomes and their order; [release decisions](maintenance.md#release-decisions) own required scope/servicing rulings. [Post-release enhancements](maintenance.md#post-release-enhancements) have a separate backlog. Development commits and branch pushes can precede release acceptance. Private session/model execution notes are governed by `AGENTS.md` and are not publication evidence.
...
- [ ] R1–R8's pre-tag implementation and acceptance criteria in `docs/maintenance.md` are complete, and D1–D3 have recorded rulings. R7's tag-dependent archive binding and publication follow section 5 below.
- [ ] Gate evidence covers the exact candidate under the affected-input/reuse rules in `AGENTS.md`. A commit, push or session change alone does not invalidate a matching result; changed relevant inputs require the affected checks again.
...
- [ ] `make test-publication-records` (or `make test-publication-records-parallel JOBS=<n>`, every registered case exactly once) passes, including schema-2/3 compatibility, immutable references, interrupted record/head durability, complete ancestor-creation authority and legacy recovery exclusion. Paired session tests cover directory retries, local mount and exact target-state checks after the last authorization, Core death and deterministic child-exit races. These fixtures and filesystem-only guest experiments do not close actual Core/FAT recovery acceptance.
- [ ] New invocation-start authority atomically binds intent, context, exact original configuration and scoped recreation permission with preservation policy. Mixed record schemas and historical non-retroactivity pass; rejected model-only retries cannot release a worker. Sealed original-effect/content-classification tests cover prior/desired/absence/conflict and preserve source versus signed-byte provenance. Fresh-attempt execution still needs separate lineage, live custody, signer, root-obligation and completion evidence.
- [ ] `make test-publication-context` passes; the package/container check verifies the installed Python/Btrfs binding API. Record actual Btrfs/FAT acquisition and root-replacement/remount evidence separately from simulated platform/command-wrapper tests. Context must precede launch; changed signing policy and failed retained/staged signature checks must block execution. Context equality alone does not authorize a fresh recovery write over conflicting content.
...
- [ ] `make test-package-catalog` passes for the same package reader/database format, including silent read loss, source revalidation, configured root/database context and missing payload. Its successful catalog is an observation, not a completed producer invocation or mandatory-entry policy.
- [ ] Producer patches pass `make test-integration` and `make test-build-settings` with the pinned source, plus `make test-native`, `make test-description`, `make test-output-expectations` and `make test-producer-session` with that source, JDK 25 and the pinned `LIMINE_NATIVE_JSON_JAR`. Run native description queries against the built ELF using `LIMINE_NATIVE_EXECUTABLE` as well. Record source/patch/toolchain bindings and inspect the package payload, including the shared shell helpers and Jackson license/notice files. Component and read-only query fixtures do not replace mutating packaged-command/FAT, architecture or boot acceptance; development 1.38.0-1.7 is not yet admitted for lifecycle activation. Exercise inode eviction, remount and reboot recovery on actual FAT: persisted Linux inode numbers alone are not durable recovery identities.
...
Hermetic tests do not prove firmware behavior. The matrix below is the minimum. Every row is recorded with `tests/acceptance-capture.sh`, which writes firmware/package state before, the full terminal transcript, and state after. Run it from a checkout on the target machine as `sudo bash tests/acceptance-capture.sh <row> -- <command>`, through `sudo` from your own login rather than from a root shell started with `su`, so the recorder can identify the checkout owner and commit. For a firmware-menu step, run it without a command after reboot.

Raw captures land in `./acceptance-records/` and stay private. Share them privately with the reviewer to determine pass/fail. The recorder omits DMI serials/UUIDs, MAC or NVMe device-path nodes, recovery keys and raw firmware backup payloads, but retains partition identifiers, boot IDs, lifecycle records and command transcripts. Review does not make those raw captures suitable for publication. Publish only sanitized summaries under `docs/acceptance/`, naming the candidate/source and package versions, tested procedure, outcome, coverage and limitations; exclude raw transcripts and machine-private identifiers.

The current recorder binds the checkout commit/dirty state and compares the installed command and shell modules byte for byte. **R8 requires extending that binding to every installed runtime helper, including `publication-context.py`, before accepting the managed runtime.** A dirty checkout, mismatching payload or incomplete component binding satisfies no affected row. Package tests already check the Python helper and its installed API; those results do not replace the hardware recorder's candidate binding.
...
3. Check the producer set and the stale-copy condition before building anything: `pacman -Q limine-mkinitcpio-hook limine-snapper-sync sbctl efibootmgr` must show Limine mkinitcpio 1.38.0-1.1 or integrated 1.38.0-1.2, snapshot sync 1.31.0-1.1, sbctl 0.18-2, and efibootmgr at 18 or newer. Record which audited build is used; evidence from one does not automatically establish the other. `ls /usr/local/bin/omasecboot /usr/local/lib/omasecboot` must report both missing. Other producer versions need re-audit before acceptance.
...
| A9 | Restore a snapshot taken after A4 through `limine-snapper-restore` with the lifecycle active | Destination runtime, keys and recovery authority proved before root replacement; after reboot the managed state and artifacts are proved, and the next kernel update succeeds. Unsafe/pre-activation destinations have a separately recorded pre-mutation refusal or the supported migration disposition | Yes |
...
| A5 | Record the candidate's selected installed kernel package and its normal update path. For the Omarchy 4.0.4 default, use `sudo bash tests/acceptance-capture.sh A5 -- pacman -S --noconfirm linux-omarchy` (if Omarchy's update guard refuses, use `omarchy update` instead), then reboot and `sudo bash tests/acceptance-capture.sh A5-boot -- omasecboot status`. A different supported kernel/profile uses its recorded package, not an assumed name |
