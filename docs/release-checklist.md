...
- [ ] `make test` passes on the candidate (17 suites). Record the date, host, and elapsed time.
...
- [ ] `tests/package-root.sh` passes in a disposable Arch container (CI, or `docker run --rm -e OMASECBOOT_DISPOSABLE_ROOT=1 -v "$PWD:/src:ro" archlinux:base-devel bash -c 'pacman -Syu --noconfirm --needed git jq diffutils util-linux libarchive && cp -r /src /work && cd /work && bash tests/package-root.sh'`).
- [ ] The built package was inspected by hand once: `pacman -Qip`, `pacman -Qlp`, and `bsdtar -tvf` output reviewed for the payload, modes, and dependencies recorded in `docs/maintenance.md`.
