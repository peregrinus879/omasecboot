...
- [ ] Producer patches pass `make test-integration` and `make test-build-settings` with the pinned source, plus `make test-native` and `make test-description` with that source and JDK 25. Run native description queries against the built ELF using `LIMINE_NATIVE_EXECUTABLE` as well. Record source/patch/toolchain bindings and inspect the package payload, including the shared shell helpers. Component and read-only query fixtures do not replace mutating packaged-command/FAT, architecture or boot acceptance; development 1.38.0-1.5 is not yet admitted for lifecycle activation.
