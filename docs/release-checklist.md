...
- [ ] Producer patches pass both `make test-integration` with the pinned source and `make test-native` with that source and JDK 25. Record source/patch/toolchain bindings and inspect the resulting producer package payload. Java component fixtures do not replace packaged-command/FAT, architecture or boot acceptance; development 1.38.0-1.3 is not yet admitted for lifecycle activation.
