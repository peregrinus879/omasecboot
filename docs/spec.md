...
5. Under the lock: the managed settings, then `enabled`, then regeneration of the OS entries when they carry hashes. Then the pass, which enables the watchers.
6. A warning when the machine has no fallback loader, and on the first run the advice to take a snapshot.
