...
The seal holds two files together, and either can change outside the Limine tools, where no hook runs. An edit of `limine.conf` without re-enrollment stops Limine from booting (D2). A plain copy of the raw executable over the primary loader leaves a loader the firmware refuses once Secure Boot is on, and Omarchy's installer leaves a pacman hook that makes that copy after every Limine upgrade, after upstream has sealed and signed [C7]. The package ships a systemd path unit template. `setup` enables one instance for the machine's `limine.conf` and one for its primary loader, `remove` disables them, and on a change the pass checks the seal and rebuilds the loader only on a mismatch. Nothing is generated and nothing stays resident. The watchers' pass waits for a running pacman to finish, five minutes at most, so it judges what the transaction's last hook left behind; a lock file without a pacman process is a crashed pacman's and is not waited for. A stop of the service, as at the shutdown after an update, signals that pass alone, and the pass ignores it and finishes, because a loader left raw does not boot. systemd merges changes that arrive while the triggered service runs [C5], so the check looks at the file's current state and repeats, three rounds at most, until the file held still across a round; the service's start rate limit is off, so a burst of saves cannot disable the watcher.
...
Read-only. It reports the firmware state and whether the user's keys are enrolled, the managed settings, the loader proof, the fallback, the signing keys, every signable file, harmful sbctl rows, stale path hashes of OS entries, unsigned history files as a count, the Windows entry, the hook, the watchers, leftovers and `needs-attention`. On a machine that is not set up it still names leftovers of an earlier install, and it reports a `setup` or `remove` that stopped half way, which `settings-originals` without `enabled` shows, and names the two commands that finish it. It ends with one next step, chosen by what repairs the worst problem seen: `sign`, `setup`, or nothing this tool runs. Anything that could not be read belongs to the last kind. `--quiet` only sets the exit status.
...
| The primary loader is replaced outside the Limine tools: Omarchy's installer hook after a Limine upgrade [C7], or a copy by hand | Boots, the loader is raw | The firmware refuses the raw loader | Secure Boot off, run `sign` | The loader's watcher rebuilds it once pacman is done; `status` error until then |
