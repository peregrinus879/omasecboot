...
Omarchy's update sequence has no step that can hold its restart prompt [C6]. `needs-attention` and the red line are the visible signal; the update step in [omarchy-integration.md](omarchy-integration.md) runs `omasecboot status --quiet`.
...
| A `limine.conf` stands where Limine reads first [C1], or an OS entry still carries a path hash | The loader is sealed over a file Limine does not read, or the entry is stale | Same, or a warning that waits for a key | Remove the stray file; `setup` regenerates the entries, and the user takes the hash off an entry written by hand | `setup` refuses beside a shadowing file, goes on only when the hashes are gone and lists the ones that remain; `status` reports both |
