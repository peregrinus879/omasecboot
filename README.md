...
`make test-description LIMINE_ENTRY_TOOL_SOURCE=... LIMINE_NATIVE_JAVA_HOME=...` exercises the actual wrapper/query, read-only behavior, native configuration/association and reader-generation contracts. Add `LIMINE_NATIVE_EXECUTABLE=/absolute/path/to/built/limine-entry-tool` to run the queries against the compiled ELF; deterministic reader and publication helpers still use the actual Java classes. `LIMINE_DESCRIPTION_KEEP=1` retains fixtures. Shell and Java suites apply all ordered patches and verify intermediate and final source bindings; these integration targets remain separate from the aggregate and CI.

The shell counterpart is `limine-mkinitcpio --describe-build PACKAGE VERSION [PACKAGE VERSION ...]`. It describes explicit declaration-derived rows using the producer's actual layer parser, inherited settings, name cleaning, mode/fallback predicates and option tokenization. It performs no initialization, authentication, locks, hooks, sbctl/Snapper query or build. JSON records as-read input digests and incomplete observation, including failed or partial enumeration. Known nonregular optional configuration, including links to `/dev/null`, is skipped as in production. Additional UKI options can override generated arguments, and the existing cmdline helper predicates do not validate probe exit statuses; both effects remain explicitly unresolved. `make test-build-settings LIMINE_ENTRY_TOOL_SOURCE=...` checks the real query and original/composed producer argv in fixtures; `LIMINE_BUILD_SETTINGS_KEEP=1` retains evidence. Native operation mapping and core's complete-output consumer remain release work.
