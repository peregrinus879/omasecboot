...
| `Limine path hashes are stale; Limine refuses these entries once Secure Boot is on` | An entry in `limine.conf` carries a hash of a file that has since changed, usually a UKI signed after limine-entry-tool hashed it | Run `sudo limine-mkinitcpio` for the OS entry, or `sudo limine-snapper-sync` for snapshot entries, then rerun the command; `status` lists the entries |
...
| `Firmware and plan observation does not match a supported setup state`, `The state N instruction boundary could not be proved`, or `... did not complete`, followed by `Observed:` lines | A read-only predicate refused, or a transaction failed. The lines show the mode variables, the local keys, the lifecycle, and PK, KEK, db, and dbx against the backup and the plan. A `Lifecycle: recovery-required` line means a transaction failed and `repair` is the only next step. When PK, KEK, and db all read `exact` against the backup before the PK delete, or against the plan after enrollment, the refusal came from another check (plan export, boot artifacts, or the Windows gate) whose message appears above | Follow the lifecycle line first. KEK, db, or dbx changed after a PK-only delete means this firmware is not supported: restore its factory keys, do not repeat the PK delete there, and report. db or dbx changed after enrollment leaves the OmaSecBoot-signed Linux boot path unaffected: keep the recorded plan and report; a plan refresh is a deferred item in the ledger |
...
Every `setup` and `sign` performs the same sequence: repair the managed `/etc/default/limine` settings (`ENABLE_VERIFICATION=no`, `ENABLE_ENROLL_LIMINE_CONFIG=yes`), regenerate the Limine entries through `limine-mkinitcpio` and `limine-snapper-sync` when they carry path hashes (setup only, once; hashes written from unsigned files would otherwise go stale when the files are signed, and Limine refuses a stale hash under Secure Boot), enroll the current `/boot/limine.conf` checksum into `/EFI/limine/limine_x64.efi` and, when Limine's fallback loader `/EFI/BOOT/BOOTX64.EFI` is deployed (`ENABLE_LIMINE_FALLBACK` is `yes` or unset in limine-entry-tool's configuration, or the file exists), into that fallback as well, verify the embedded checksum directly in each, clean stale sbctl tracking, sign and track every discovered non-Microsoft EFI artifact as the last mutation, then read everything back. Each Limine target is rebuilt from the unsigned package executable in a staging file, enrolled, signed, verified, and synced before it atomically replaces the target, so an unsigned or wrongly enrolled Limine is never published. Higher-priority `limine.conf` candidates on the ESP that could shadow `/boot/limine.conf` are rejected.
