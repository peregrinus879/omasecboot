...
- **The fallback loader** `EFI/BOOT/BOOTX64.EFI`. It stays the raw copy upstream deploys; where there is none, `setup` offers to add one through upstream's own tool. The firmware refuses it while Secure Boot is on, and with Secure Boot off it is your rescue loader ([If the machine does not start](#if-the-machine-does-not-start)). The comments in upstream's `/etc/limine-entry-tool.conf` suggest signing it by hand; `sign` undoes that, because a fallback that is signed but not sealed would start under Secure Boot without enforcing `limine.conf`. "Raw" means unsealed and unsigned: `status` calls the fallback upstream's raw copy only when its bytes are, and otherwise says what sbctl can tell, no seal and no signature by your key, which does not rule out a signature by another key the firmware trusts.
...
If there is no `limine_x64.bak`, or tar reports an error, a live system that has the `limine` package holds a raw loader at `/usr/share/limine/BOOTX64.EFI` (`pacman -Qo` that path says whether yours does; with a network, `pacman -Sy limine` puts it there): `cp /usr/share/limine/BOOTX64.EFI /mnt/EFI/limine/limine_x64.efi`. That is the raw loader of the live system's Limine version; it serves for the one start, and `sign` then rebuilds the loader from the installed one. Do not copy `/mnt/EFI/BOOT/BOOTX64.EFI` over it, which on a machine installed beside another system may be that system's. Stop at the first command that reports an error: a loader that was not replaced is the one that refused to start. When the loader is in place, `umount /mnt`, then start Omarchy with Secure Boot off. Where tar reported an error, move the damaged copy aside first, `sudo mv /boot/EFI/limine/limine_x64.bak /boot/EFI/limine/limine_x64.bak.damaged`, then run `sudo limine-install`, which deploys the installed package's loader and writes a fresh copy beside it: `sign` builds the loader from that copy while one exists, refuses while it cannot read it, and builds from the package's executable without one. Then run `sudo omasecboot sign`, or go on to [Removing it](#removing-it).
...
`remove` returns the boot files to stock through upstream's own `limine-install`, which writes upstream's fallback loader over whatever stands at `EFI/BOOT/BOOTX64.EFI` when `ENABLE_LIMINE_FALLBACK=yes` is in effect, as every Limine upgrade does; that is Omarchy's setting on a single-boot install, and `no` is what its installer writes beside another system. Where another system's loader stands there with `yes` in effect, `remove` says so before its question. To keep that loader, answer no, set `ENABLE_LIMINE_FALLBACK=no` in `/etc/default/limine`, the layer that wins over the others, and run `remove` again.
