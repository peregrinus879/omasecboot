...
`make test-native LIMINE_ENTRY_TOOL_SOURCE=/absolute/path/to/pinned/source LIMINE_NATIVE_JAVA_HOME=/absolute/path/to/jdk25 LIMINE_NATIVE_JSON_JAR=/absolute/path/to/jackson-core-3.2.2.jar` compiles and runs the actual Java publisher classes against original and patched sources. The source manifest binds every compiled Java file and the protocol decoder's jar. The suite needs JDK 25, Bubblewrap, jq, patch, ShellCheck, a C compiler and the usual Bash/coreutils tools; it downloads nothing and keeps configuration, boot paths, HOME and XDG paths inside fixtures. Set `LIMINE_NATIVE_KEEP=1` to retain the printed fixture directory and logs. It checks detected publication failures, shared resource rendering, strict protocol decoding, pipe framing and prepared application attempts. Real sync faults and process restarts exercise exact pending-stage identities, ordered publication, finite deletion and namespace dependencies. The fixture authority does not establish the Core runtime, FAT inode lifetime, recovery after remount or boot acceptance. Like the shell-producer gate, this target is separate from `make test` and current CI.

The development producer also exposes `limine-entry-tool --describe context`, `--describe add-uki NAME SOURCE`, and `--describe add-kernel NAME INITRAMFS_SOURCE KERNEL_SOURCE [SUFFIX]`. These return versioned JSON native-operation facts without producer initialization, authentication, logging, locks or hooks. Missing resources can be described. Configuration/entry observations are as-read, not a locked filesystem snapshot; prospective native paths and names are not package inventory, shell build selection, publication success or boot proof. An explicit non-FAT ESP can be reported with its failed mutation precondition. The producer's packaged README documents the schema and exit meanings.

`make test-description LIMINE_ENTRY_TOOL_SOURCE=... LIMINE_NATIVE_JAVA_HOME=... LIMINE_NATIVE_JSON_JAR=...` exercises the actual wrapper/query, read-only behavior, native configuration/association and reader-generation contracts. Add `LIMINE_NATIVE_EXECUTABLE=/absolute/path/to/built/limine-entry-tool` to run the queries against the compiled ELF; deterministic reader and publication helpers still use the actual Java classes. `LIMINE_DESCRIPTION_KEEP=1` retains fixtures. Shell and Java suites apply all ordered patches and verify intermediate and final source bindings; these integration targets remain separate from the aggregate and CI.

The shell counterpart is `limine-mkinitcpio --describe-build [PACKAGE VERSION ...]`. Its schema-2 shell-build description includes explicit declaration-derived rows and native publication-step operands from the same mapper as the actual producer. With no pairs it describes build context only. It shares the actual layer parser, inherited settings, name cleaning, mode/fallback predicates and option tokenization, with no initialization, authentication, locks, hooks, sbctl/Snapper query or build. JSON records as-read input digests and incomplete observation, including failed or partial enumeration. Known nonregular optional configuration, including links to `/dev/null`, is skipped as in production. Additional UKI options and the existing cmdline helper predicates remain explicitly unresolved builder effects. `make test-build-settings LIMINE_ENTRY_TOOL_SOURCE=...` checks the query against original/composed builder and native-publication argv; `LIMINE_BUILD_SETTINGS_KEEP=1` retains evidence. Core's complete-active consumer remains release work.

`make test-pacman-contract` runs the installed stock pacman against disposable local package archives and databases in unprivileged Bubblewrap namespaces. It needs pacman, Bubblewrap, libarchive's `bsdtar`, a C compiler, ShellCheck and the usual Bash/coreutils tools; it uses no network, host package database, signing keys or boot files. The suite checks declaration completeness pitfalls, matching pre/post hook targets, early and terminal failures, and retries. Its fixture-only stdio shim reproduces a silent partial file-list read from real libalpm. Set `TMPDIR` to an existing private scratch directory and `PACMAN_CONTRACT_KEEP=1` to retain its printed fixture directory and transcripts. This separate integration target is not part of `make test` or the current CI matrix; it establishes stock-tool contracts, not production lifecycle or boot acceptance.

`make test-package-catalog` exercises the core catalog reader against real fixture package databases, including missing installed files, alternate roots/databases, source drift and silent CLI omissions. It uses the same compiler/namespace tools plus pacman-conf, jq, GNU awk/findutils and iconv. `PACKAGE_CATALOG_KEEP=1` retains its observations. The catalog is a paired-runtime prerequisite; it does not yet participate in lifecycle activation. An installed kernel is a catalog row, while required entries also depend on producer policy and explicit recorded native intent.
...
New managed additions atomically record their original intent, context, configuration bytes and narrowly scoped missing-file recreation permission together with preservation policy. Historical records retain their original limits. The journal tests also exercise sealed original-effect resolution and content classification: known prior bytes, desired retained bytes, explicitly permitted absence, or conflict. Those classifications do not by themselves authorize a recovery write. A preparatory recovery attempt now also records its exact private copies of the original bytes, a freshly collected context with the attempt's signing policy, one classified fresh observation per original target, disposable sibling stages from those copies, the resulting readiness plan and the applied effects with their closing result; the attempt still cannot complete, so it seals failed with its writes preserved, and complete-active proof and whole-root scheduling remain separate work.
