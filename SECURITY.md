...
- A way for an unprivileged user to make the tool, its hook or its watchers change a boot file, a setting or the firmware's variables.
- Key material, or the private details of a machine, leaving it through the tool, the recorder or the share step.
- A refusal that can be bypassed: the guards around the Platform Key, the fallback loader and another system's boot files.
...
The latest release and `main`. Before the first release, `main` alone.
