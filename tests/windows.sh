...
# managed entry in limine.conf and when it may be touched, the note about a
# chainload entry beside BitLocker, the BootNext request, and the encryption
# acknowledgement and reminders before anything changes what Windows measures
# at boot.
...
add_chainload_entry() {
  printf '\n/%s\n### This EFI entry is auto-generated by limine-entry-tool\ncomment: %s\ncomment: order-priority=20 \nprotocol: efi\npath: %s\n' "$1" "$1" "$2" >>"$FIX/esp/limine.conf"
}

# Entries are named as limine-remove-entry takes them (C2): parents first,
# without the marker of an expanded directory, with the position among the
# entries of that name, and of several chainloads under one name the first.
# Keys have no case (C1), a path may carry a hash, and a line that only
# mentions such a path is no path.
chainloads_are_listed_as_upstream_names_them() {
  [[ -z $(list_windows_chainloads) ]] || fail_test "a stock limine.conf listed: $(list_windows_chainloads)"
  printf '\n# path: boot():/EFI/Microsoft/Boot/bootmgfw.efi\n/Notes\ncomment: path: boot():/EFI/Microsoft/Boot/bootmgfw.efi\nmodule_path: boot():/EFI/Microsoft/Boot/bootmgfw.efi\n' >>"$FIX/esp/limine.conf"
  [[ -z $(list_windows_chainloads) ]] || fail_test "a line that only mentions the path listed: $(list_windows_chainloads)"
  add_chainload_entry 'Windows Boot Manager' 'uuid(0a1b2c3d-1111-2222-3333-444455556666):/EFI/Microsoft/Boot/bootmgfw.efi'
  add_chainload_entry 'Windows Boot Manager' 'uuid(0a1b2c3d-7777-2222-3333-444455556666):/EFI/Microsoft/Boot/bootmgfw.efi'
  add_chainload_entry 'Other system' 'boot():/EFI/other/grubx64.efi'
  printf '\n/+Other systems\n  //Windows 11\n  PROTOCOL: efi\n  IMAGE_PATH: boot():/EFI/Microsoft/Boot/BOOTMGFW.EFI#%0128d\n' 0 >>"$FIX/esp/limine.conf"
  [[ $(list_windows_chainloads) == $'1\tWindows Boot Manager\n1\tOther systems/Windows 11' ]] || fail_test "listed: $(list_windows_chainloads)"
  # A chainload that shares its name with an entry before it, here this tool's.
  printf '%s\n' "$(windows_entry 'Windows Boot Manager')" >>"$FIX/esp/limine.conf"
  add_chainload_entry 'Windows' 'boot():/EFI/Microsoft/Boot/bootmgfw.efi'
  [[ $(list_windows_chainloads) == $'1\tWindows Boot Manager\n1\tOther systems/Windows 11\n2\tWindows' ]] || fail_test "beside the entry of this tool: $(list_windows_chainloads)"
}

# Beside a BitLocker volume the two ways of starting Windows do not mix, and
# only the firmware's has a record of staying quiet (D11, C10): the report,
# the Windows report and windows setup say so and name upstream's command. The
# entry is upstream's or the user's and stays; without BitLocker it does no
# harm and nothing is said.
chainload_beside_bitlocker_is_noted() {
  local command
  add_windows
  run_cli setup || fail_test "setup failed: $(<"$FIX/run/output")"
  add_chainload_entry 'Windows Boot Manager' 'uuid(0a1b2c3d-1111-2222-3333-444455556666):/EFI/Microsoft/Boot/bootmgfw.efi'
  add_chainload_entry 'Other system' 'boot():/EFI/other/grubx64.efi'
  run_cli sign || fail_test "sign after the edit failed: $(<"$FIX/run/output")"
  for command in 'windows setup' 'status' 'windows status'; do
    # shellcheck disable=SC2086 # The command and its action are two words.
    run_cli $command || fail_test "${command} failed beside a chainload entry: $(<"$FIX/run/output")"
    [[ $(<"$FIX/run/output") == *'chainload entry for Windows, "Windows Boot Manager". BitLocker measures'*'Start Windows through the firmware only'*"sudo limine-remove-entry 'Windows Boot Manager' takes"* ]] ||
      fail_test "${command} did not say it: $(<"$FIX/run/output")"
    [[ $(<"$FIX/run/output") != *'Other system'* ]] || fail_test "${command} named another system's loader: $(<"$FIX/run/output")"
  done
  grep -q 'bootmgfw.efi' "$FIX/esp/limine.conf" || fail_test "the chainload entry was taken out"
  [[ $(entry_count) == 1 ]] || fail_test "the entry of this tool: $(entry_count)"
  # Volumes that cannot be listed are no answer: the condition is said with it.
  : >"$FIX/run/lsblk-fails"
  run_cli status || fail_test "status failed: $(<"$FIX/run/output")"
  [[ $(<"$FIX/run/output") == *'"Windows Boot Manager". If Windows is encrypted, BitLocker measures'* ]] || fail_test "unlisted volumes: $(<"$FIX/run/output")"
  rm "$FIX/run/lsblk-fails"
  # The Windows report says it without a target in the firmware as well.
  rm "$FIX"/efivars/Boot0000-*
  run_cli windows status || fail_test "windows status failed: $(<"$FIX/run/output")"
  [[ $(<"$FIX/run/output") == *'does not hold exactly one active Windows Boot Manager entry'*'chainload entry for Windows'* ]] || fail_test "windows status without a target: $(<"$FIX/run/output")"
  printf '/dev/nvme0n1p1 vfat\n/dev/nvme0n1p5 btrfs\n' >"$FIX/run/lsblk"
  run_cli status
  [[ $(<"$FIX/run/output") != *'chainload entry'* ]] || fail_test "a machine without BitLocker was told about it: $(<"$FIX/run/output")"
}

# The name comes from limine.conf and the command is pasted into a shell: it is
# quoted so that the shell hands upstream the name as it stands, with the
# position where an earlier entry shares it.
printed_command_survives_the_shell() {
  local output name
  add_windows
  add_chainload_entry '+Other systems' 'boot():/EFI/other/grubx64.efi'
  printf "  //Win'dows \$(id) \`id\`\n  protocol: efi\n  path: boot():/EFI/Microsoft/Boot/bootmgfw.efi\n" >>"$FIX/esp/limine.conf"
  add_chainload_entry 'Spare' 'boot():/EFI/other/spare.efi'
  add_chainload_entry 'Spare' 'boot():/EFI/Microsoft/Boot/bootmgfw.efi'
  output=$(note_windows_chainloads)
  [[ $output == *"sudo limine-remove-entry 'Spare' 2 takes"* ]] || fail_test "the position: ${output}"
  name=$(sed -n "s/.*sudo limine-remove-entry \\('Other systems.*'\\) takes that entry out.*/\\1/p" <<<"$output")
  [[ -n $name ]] || fail_test "no command for the nested entry: ${output}"
  [[ $(eval "printf '%s' ${name}") == "Other systems/Win'dows \$(id) \`id\`" ]] || fail_test "the shell reads the name as: $(eval "printf '%s' ${name}")"
}

# Secure Boot's state is part of what Windows measures, and turning it off is
# where BitLocker asked on the recorded machine (C10): remove says so where it
# asks for that, beside Windows only.
remove_reminds_of_the_recovery_key() {
  run_cli setup || fail_test "setup failed: $(<"$FIX/run/output")"
  set_mode_variable SecureBoot 1
  run_cli remove && fail_test "remove ran with Secure Boot on"
  [[ $(<"$FIX/run/output") != *'recovery key'* ]] || fail_test "a machine without Windows was told about it: $(<"$FIX/run/output")"
  add_windows
  run_cli remove && fail_test "remove ran with Secure Boot on"
