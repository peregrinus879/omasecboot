...
[[ $status_hooks -eq 6 ]] || fail "status hook list could not be read from lib/status.sh (${status_hooks})"
shadow_hooks=0
while IFS= read -r status_hook; do
...
[[ $shadow_hooks -eq 3 ]] || fail "status shadow list could not be read from lib/status.sh (${shadow_hooks})"
...
for hook in "$limine_pre_hook" "$limine_post_hook"; do
  [[ $(stat -Lc '%a' "$hook") == 755 ]] \
    || fail "Limine hook has the wrong installed mode: ${hook##*/}"
done
...
[[ ! -e "$canonical_state" ]] \
  || fail "install created the durable state directory as package content"
[[ -f "$tmpfiles_conf" && $(stat -Lc '%a' "$tmpfiles_conf") == 644 ]] \
  || fail "tmpfiles declaration is missing or has the wrong mode"
grep -Fxq 'd /var/lib/omasecboot 0755 root root -' "$tmpfiles_conf" \
  || fail "tmpfiles declaration does not declare the durable state directory"
grep -Fxq 'f /var/lib/omasecboot/repair.lock 0644 root root -' "$tmpfiles_conf" \
  || fail "tmpfiles declaration does not declare the stable repair lock"
[[ $(grep -c -v -E '^(#|$)' "$tmpfiles_conf") -eq 2 ]] \
  || fail "tmpfiles declaration carries an unexpected entry"
...
[[ -f "${canonical_state}/repair.lock" ]] \
  || fail "idempotent reinstall removed the stable repair lock"

if make -s -C "$ROOT_DIR" uninstall DESTDIR="$STAGE_DIR" PREFIX="$PREFIX" \
  "${MAKE_INSTALL_PATHS[@]}" > "${STAGE_DIR}/uninstall.out" 2>&1; then
  fail "source uninstall succeeded outside the package removal contract"
fi
grep -Fq 'Refusing source uninstall; remove the omasecboot package with pacman' \
  "${STAGE_DIR}/uninstall.out" || fail "blocked uninstall omitted its package boundary"
