...
# The OS entry as limine-entry-tool writes it: an unindented body with the
# machine id and order-priority on a comment line (C2).
write_limine_conf() {
  local hash=''
...
# Omarchy's template, which omarchy-refresh-limine copies over limine.conf
# before limine-update fills it (C6): settings, no entry.
...
# limine.conf as an Omarchy machine's reads after limine-update and
# limine-snapper-sync (C2, C10): the template's settings, the OS entry with two
# kernels and a nested snapshot block, a limine-scan chainload entry and the
# fallback entry, the last two ordered by priority. SNAPSHOTS more snapshots
# make the file as long as a machine with a full history has it.
write_omarchy_limine_conf() {
  local snapshots=${1:-1} n
  write_omarchy_limine_template
  cat >>"$FIX/esp/limine.conf" <<'EOF'
...
comment: machine-id=0123456789abcdef0123456789abcdef order-priority=50 
  ...
  comment: Kernel version: 7.2.5-3-omarchy
  comment: kernel-id=linux-omarchy 
  protocol: efi
  path: boot():/EFI/Linux/omarchy_linux-omarchy.efi
  cmdline: quiet splash

  //linux
  ### This kernel entry is auto-generated by limine-entry-tool
  comment: Kernel version: 7.2.3-arch1-3
...
EOF
  for ((n = snapshots; n >= 1; n--)); do
    cat >>"$FIX/esp/limine.conf" <<EOF
     ///$((n + 7)) │ 2026-09-22 16:53:38
     comment: snapshot ${n}
     ////linux-omarchy
     comment: Kernel version: 7.2.5-3-omarchy
     comment: kernel-id=linux-omarchy
     protocol: efi
     path: boot():/0123456789abcdef0123456789abcdef/limine_history/omarchy_linux-omarchy.efi_sha256_$(printf 'a%.0s' {1..64})#$(printf '0%.0s' {1..128})
     cmdline: quiet splash
     ////linux
     comment: Kernel version: 7.2.3-arch1-3
     comment: kernel-id=linux
     protocol: efi
     path: boot():/0123456789abcdef0123456789abcdef/limine_history/omarchy_linux.efi_sha256_$(printf 'a%.0s' {1..64})#$(printf '0%.0s' {1..128})
     cmdline: quiet splash
EOF
  done
  cat >>"$FIX/esp/limine.conf" <<'EOF'
...
protocol: efi
path: uuid(0a1b2c3d-1111-2222-3333-444455556666):/EFI/Microsoft/Boot/bootmgfw.efi
...
# bytes before the data, non-volatile and authenticated for the key variables
# and volatile for the firmware's built-in defaults, which it sets at every
# boot (UEFI 2.10, 3.3 and 32.6.1). The lists travel as base64 because they
# hold NULs.
write_key_variable() {
  local attributes='\x27\x00\x00\x00'
  [[ $1 != *Default ]] || attributes='\x06\x00\x00\x00'
  { printf '%b' "$attributes"; base64 -d <<<"$2"; } >"$(key_variable_path "$1")"
}
...
# Overrides that depend on a run/ flag read it when the library loads: a
# case that sets such a flag after fixture_machine redefines the function
# itself instead.
fixture_overrides() {
...
content=$(<"$file")
# The real tool refuses a file without the marker and a checksum that is not
# 128 hex digits (C1).
[[ $content == *"$marker"* && $sum =~ ^[0-9a-f]{128}$ ]] || exit 1
content=${content%"$sig"}
...
# The pre-hook puts the primary back from the backup, and without one only
# resets the checksum in place, through limine enroll-config --reset (C2).
if [[ -f $limine_dir/limine_x64.bak ]]; then
  tar -xf "$limine_dir/limine_x64.bak" -C "$limine_dir" limine_x64.efi
else
  limine enroll-config --reset "$primary"
fi
case $name in
  limine-mkinitcpio)
    if [[ ! -e $FIX/run/uki-build-fails-silently ]]; then
      # Every UKI of upstream's own entries, one per installed kernel (the
      # fixture's carry the "omarchy_" prefix), rebuilt, signed at build and
      # re-hashed in its entry, whatever the entry's indentation; an entry
      # upstream did not write is left as it is (C2).
      for uki in $(grep -o -E '^[[:space:]]*path: boot\(\):/EFI/Linux/omarchy_[^#[:space:]]+' "$FIX/esp/limine.conf" | sed 's|.*/||' | sort -u); do
        printf 'UKI rebuilt %s\n' "$RANDOM" >"$FIX/esp/EFI/Linux/$uki"
        [[ ! -e $FIX/sbctl/keys || -e $FIX/run/uki-arrives-unsigned ]] || sbctl sign "$FIX/esp/EFI/Linux/$uki"
        hash=''
        # A hash only for a value of yes, in any case (C2).
        [[ $(setting ENABLE_VERIFICATION | tr 'A-Z' 'a-z') != yes ]] || hash="#$(b2sum <"$FIX/esp/EFI/Linux/$uki" | cut -d' ' -f1)"
        sed -i "s|^\([[:space:]]*path: boot():/EFI/Linux/${uki//./\\.}\)\(#[0-9a-f]*\)\?$|\1${hash}|" "$FIX/esp/limine.conf"
      done
  ...
  limine-install)
    ...
    # With FIND_BOOTLOADERS=yes upstream adds its "EFI fallback" entry for a
    # fallback loader that exists, once, under the lock and before the
    # post-hooks (C2).
    if [[ $(setting FIND_BOOTLOADERS) == yes && -e $fallback ]] && ! grep -q '^/EFI fallback$' "$FIX/esp/limine.conf" 2>/dev/null; then
      printf '\n/EFI fallback\n### This EFI entry is auto-generated by limine-entry-tool\ncomment: Default EFI loader\ncomment: order-priority=10 \nprotocol: efi\npath: boot():/EFI/BOOT/BOOTX64.EFI\n' >>"$FIX/esp/limine.conf"
    fi
    ;;
...
EOF
  ...
  # asks with pgrep -x, which exits 0 when a process by that name runs
  # (procps pgrep(1)).
...
EOF
  # pacman -D --asexplicit marks a package as explicitly installed and prints
  # nothing on success (pacman 7, pacman(8)); the tool reads no output.
